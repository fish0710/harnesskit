...
10. Harness 将选中的机器契约作为远端 Gate 契约执行。
...
16. 宿主聚合 Gate 结果。
17. `fail` 或 `error` 生成受限诊断，反馈给原 agent 沙箱继续下一轮。
18. `blocked` 停止自动循环，等待人工 verdict。
19. 超出轮数、时间、token 或重复失败阈值时升级。
20. `pass` 时 publisher 写回门禁验证过的精确候选快照。
21. run manifest 持续记录 host 事件、agent/gate sandbox id、attempt 结果和
...
`harness preflight gate` is the same readiness barrier that Daytona-backed
`run` and `fix` execute before creating an Agent sandbox. It does not publish
files. It creates a fresh Gate sandbox from the configured Gate snapshot,
uploads the current host workspace, runs `gateSetup`, applies the same
loopback-aware network policy as `run`, and executes selected remote contracts
through host-owned GateCore.
...
contracts/config/setup before the implementation agent starts. Product-red
contract failures do not block Agent startup because they may be the work the
Agent is supposed to repair.
...
multipart 上传。交互 shell 中不要去根目录查 `/workspace`；该 SDK 相对路径在
`daytona` 用户下解析为 `/home/daytona/workspace/candidate`。
...
| 视角 | 路径 |
|---|---|
| Harness logical workspace root | `/workspace/candidate` |
| Daytona SDK workspace path | `workspace/candidate` |
| Agent interactive shell workspace | `/home/daytona/workspace/candidate` |
...
- 通过后发布精确候选字节；
- `npm run check` 覆盖当前单元和集成测试；
- `node --test dist/test/daytona-environment.test.js` 覆盖 `gateSetup` 在候选覆盖后执行、
  loopback HTTP 不启用 `networkBlockAll`、Gate 反馈和重复失败升级的回归测试；
