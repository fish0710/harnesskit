...
9. Harness 按执行域拆分契约：`miniprogram` 走 host-local，其它机器门禁走远端
   gate 沙箱。
10. 如果存在远端契约，Harness 为当前 attempt 创建一个全新的 gate 沙箱。
11. 宿主在 gate 沙箱中组装 baseline、候选文件和受保护测试资产。
12. gate 沙箱在候选文件和受保护资产都组装完成后执行 `gateSetup`。
13. 若远端契约不需要 loopback HTTP，gate 沙箱关闭出站网络并执行宿主发出的合约命令。
...
15. 如果存在 host-local 契约，宿主将同一份 `CandidateSnapshot` materialize 到
    临时目录，恢复受保护文件，运行 `GateCore` 的 host-local 合约，然后删除临时目录。
16. 宿主聚合远端 gate 和 host-local gate 的所有结果。
