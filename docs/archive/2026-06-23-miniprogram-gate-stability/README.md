...
- `miniprogram` 插件在 managed DevTools 启动成功后，会等待
  `ws://127.0.0.1:<autoPort>` 自动化协议返回 `Tool.getInfo.SDKVersion`，再启动
  runner。
...
- managed mode doctor 使用临时小程序项目执行 `cli islogin`、`cli auto` 并等待
  automation WebSocket 返回 `SDKVersion`；
...
- `connect` mode preflight 会校验 `wsEndpoint` 并在本地执行路径下检查 automation
  WebSocket 协议 readiness；
...
## Follow-up: Protocol Readiness And Doctor Cleanup

同日二次复盘来自真实 DevTools UI 截图：preflight 后微信开发者工具停在
`harness-miniprogram-doctor`，资源管理器显示根目录 `/`，模拟器报：

```text
Error: param projectpath isundefined
```

根因是上一版 doctor 只等待 TCP 端口监听，然后立即删除临时 doctor 项目；DevTools
窗口仍停留在已删除项目上，后续会退化到根路径。二次修复：

- readiness 从“TCP 端口监听”升级为“WebSocket `Tool.getInfo` 返回
  `SDKVersion`”；
- preflight doctor 结束后执行 `cli quit`，等待 `autoPort` 释放后才删除临时项目；
- 如果 doctor 清理失败，则保留临时项目目录，避免 DevTools 指向一个已删除路径；
- doctor fixture 补齐 `compileType`、`libVersion`、`setting`、`app.wxss` 和页面样式；
- Node 20 没有全局 `WebSocket`，因此增加原生 TCP WebSocket 握手 fallback；
- 文档说明不要并发运行同一 `autoPort` 的 preflight 和实际小程序 gate。
...
该项目的 `harness preflight gate --dir contracts --json` 串行返回 `ready`，
`harness check --dir contracts --json` 返回 `pass`。preflight 后 `9420` 无残留监听，
说明 doctor 不再把 DevTools 留在临时项目窗口上。
